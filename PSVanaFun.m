%% Function version of PSVanalysis.m

function [] = PSVanaFun(timeZero,avgAuto,magGet,startAvg,stopAvg,stopData)
...
zeroTime = timeZero;            % time to zero average (s)

autoAvg = avgAuto;              % 0= use avg times below
                                % 1= detect light on from data file

get_mag = magGet;               % use to extract magnetic component of data
...
avgStart = startAvg;            % when to start data average (s)
avgStop = stopAvg;              % when to stop data average (s)
cutoff = stopData;              % when to truncate data file (s)
...
% find shortest file to use as test data
[~,minIndex] = min([files.bytes]);

testData = importdata(files(minIndex).name,'\t',1);
if autoAvg == 1
